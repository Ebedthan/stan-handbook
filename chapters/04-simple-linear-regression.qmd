...
Everything in this chapter is a template. When you write a Poisson model in
Chapter 11 or a hierarchical model in Chapter 8, you will be extending exactly
this structure.
...
- Write a complete Stan program for simple linear regression, including
  priors, likelihood, and a `generated quantities` block.
- Compile and sample from the model using `cmdstanr`, pass data as a named
  R list, and extract posterior draws with `posterior`.
...
- Compare Stan posterior summaries with `lm()` output and explain any
  differences.
...
\qquad \varepsilon_i \sim \text{Normal}(0, \sigma)
$$

This is the form you have seen in every statistics course. The residual $\varepsilon_i$ is the gap between the observed $y_i$ and the line $\alpha + \beta x_i$, which is the part the model cannot explain. Assuming those gaps are normally distributed with mean zero and constant spread $\sigma$ is what makes this *linear regression* rather than some other model.

Now read the equation differently. If $\varepsilon_i \sim \text{Normal}(0, \sigma)$, and $y_i = \alpha + \beta x_i + \varepsilon_i$, then $y_i$ is just $\varepsilon_i$ shifted by the constant $\alpha + \beta x_i$. Shifting a normal random variable by a constant shifts its mean and leaves its variance unchanged. So:
...
The right-hand side is exactly the same model, written as a direct probability statement about $y_i$ rather than about its residual. Instead of saying *"the noise around the line is normal"*, we say *"the observation itself is normal, centred on the line"*. No information is added or removed, only the perspective changes.

This second form is the one Stan uses. Every likelihood statement in Stan is written this way: as a distribution for the *observed variable*, with its mean expressed as a function of the parameters. Getting comfortable with this shift in perspective, from residuals to observations, is the single most important conceptual step in moving from classical to probabilistic modelling.

Then, there are three parameters to estimate:

| Parameter | Meaning | Constraint |
|---|---|---|
| $\alpha$ | Intercept: expected $y$ when $x = 0$ | none |
| $\beta$ | Slope: change in expected $y$ per unit increase in $x$ | none |
| $\sigma$ | Residual SD: unexplained variation | $> 0$ |

: Parameters of simple linear regression. {#tbl-params}

---

## A running example: litter decomposition

Litter decomposition is a key process in forest ecosystem nutrient cycling.
We will model the relationship between **mean annual temperature** (°C,
centred) and **decomposition rate** $k$ (yr⁻¹, log-transformed) across plots
in a West African forest-savanna mosaic.

We simulate data that are realistic for this system, but in your own work
you would replace `simulate_data()` below with a call to read your own
dataset.

```{r}
#| label: simulate-data
#| fig-cap: >
#|   Simulated litter decomposition data. Mean annual temperature (centred,
#|   °C) predicts log decomposition rate. The dashed line is the true
#|   data-generating relationship; points scatter around it with σ = 0.25.
#| fig-width: 6.5
#| fig-height: 4

set.seed(2026)

# True parameter values (we will try to recover these)
true_alpha <- 1.20   # log(k) at mean temperature
true_beta <- 0.08    # increase in log(k) per 1°C above mean
true_sigma <- 0.25   # residual SD

N <- 45  # number of plots

# Predictor: temperature anomaly (°C) relative to site mean
x <- rnorm(N, mean = 0, sd = 2.5)

# Outcome: log decomposition rate
y <- rnorm(N, mean = true_alpha + true_beta * x, sd = true_sigma)

# Collect into a data frame for plotting
decomp <- data.frame(x = x, y = y)

ggplot(decomp, aes(x, y)) +
  geom_point(colour = "#2e7d6a", alpha = 0.75, size = 2) +
  geom_abline(
    intercept = true_alpha, slope = true_beta,
    linetype = "dashed", colour = "grey40", linewidth = 0.8
  ) +
  labs(
    x = "Temperature anomaly (°C, centred)",
    y = expression(log~decomposition~rate~(yr^{-1})),
    title = "Litter decomposition across forest-savanna plots"
  )
```
...
parameters {
  ...
  real beta;  // slope
  real<lower=0> sigma; // residual SD that must be positive
}

model {
  // Priors
  alpha ~ normal(0, 10);  // weakly informative
  beta ~ normal(0, 5);    // weakly informative
  sigma ~ exponential(1);  // positive; mass on moderate values
...
generated quantities {
  ...
  vector[N] y_rep;
  // Point-wise log-likelihood for LOO-CV (Chapter 14)
  vector[N] log_lik;
...
**`parameters` block.**  
`alpha` and `beta` are unconstrained reals. `sigma` is declared with `<lower=0>` but notice that Stan will transform it internally to sample on an unconstrained scale (via a log transform) and back-transform for you.
...
// loop (equivalent, slower)
for (i in 1:N)
  y[i] ~ normal(alpha + beta * x[i], sigma);
```

Prefer the vectorised form whenever possible. Stan can apply
auto-differentiation more efficiently to vectorised expressions, and the
...
#| label: compile-real
#| include: false
# This block actually runs during render (model file is present)
mod <- cmdstan_model("../stan/simple_regression.stan")
```

### Step 2: prepare the data list
...
fit <- mod$sample(
  ...
  iter_warmup = 1000,
  iter_sampling = 1000,
  refresh = 0  # suppress iteration output in rendered doc
)
```
...
#| label: sample-real
#| include: false
# Save path for the fit — persists across chunk boundaries and re-renders.
# cmdstanr fit objects hold pointers to /tmp CSV files that vanish between
# R sessions; saving to .rds and reloading avoids that problem.
fit_path <- "../RDS/simple_regression_fit.rds"
...
With four chains of 1 000 sampling iterations each, we collect
$4 \times 1\,000 = 4\,000$ posterior draws.

::: {.callout-tip}
**Always save your fit to disk.**  
`cmdstanr` fit objects store posterior draws as CSV files in a temporary
directory (`/tmp/` on Linux/macOS). Those files disappear when the R session
ends. The `if (!file.exists(...))` is a pattern that helps with this by fitting the model once, saves it with `fit$save_object()`, and 
reloads it on subsequent renders. Delete the `.rds` file whenever you want 
to force a re-fit (e.g. after changing the model or the data). This pattern
is used in every model chapter of this handbook.
:::
...
- **`mean`** and **`median`**: posterior centre. Both should be close for
  symmetric posteriors.
- **`sd`**: posterior standard deviation, your uncertainty about the
  parameter.
- **`q5` / `q95`**: the 90% credible interval.
- **`rhat`**: $\hat{R}$. Must be $< 1.01$. Values above this, signal that
  chains have not converged to the same distribution
  [@vehtari2021rank].
- **`ess_bulk`** and **`ess_tail`**: effective sample sizes for the
  bulk and tails of the distribution. Aim for $> 400$ each. Low ESS
  means the chain is autocorrelated and you need more iterations.
...
#| label: fig-trace
#| fig-cap: >
#|   Trace plots for the three model parameters. All four chains overlap
#|   and show no trend or drift, the textbook "fuzzy caterpillar"
#|   pattern indicating healthy convergence.
#| fig-width: 8
#| fig-height: 5
#| cache: true
#| code-fold: true
...
All four chains explore the same region of the parameter space throughout the 1 000 sampling iterations, with no visible trend, drift, or separation between chains, the textbook "fuzzy caterpillar" pattern that indicates the sampler has converged. The posterior for $\alpha$ is tightly bounded between approximately 1.10 and 1.35, consistent with the marginal summary in @tbl-summary; the chains for $\beta$ fluctuate around 0.07 with a spread of roughly 0.025 to 0.125, reflecting the moderate uncertainty about the temperature slope at $N = 45$. The $\sigma$ chains oscillate between 0.20 and 0.38, with slightly wider excursions than the location parameters which is typical for variance components, which are estimated less precisely than means from the same data. The visual agreement between chains is confirmed numerically by $\hat{R} < 1.01$ and $\text{ESS}_{\text{bulk}} > 400$ for all parameters reported in the summary table above; if either condition had failed, these trace plots would have revealed the problem immediately as separated or drifting chains.


### Posterior pair plot

Pair plots reveal correlations between parameters. Strong correlation means
the sampler must navigate a curved posterior which is not a problem per se, but
worth knowing.

```{r}
#| label: fig-pairs
#| fig-cap: >
#|   Posterior pair plot. The mild negative correlation between alpha and
#|   beta is expected when the predictor is nearly (but not perfectly)
#|   centred. Sigma is nearly independent of both location parameters.
#| fig-width: 6
#| fig-height: 5.5
#| cache: true
#| code-fold: true
...
mcmc_pairs(
  ...
  off_diag_args = list(size = 0.4, alpha = 0.3)
)
```
...
## Interpreting the posterior {#sec-interpret}

```{r}
#| label: posterior-draws
#| cache: true
# Extract draws as a data frame
draws <- as_draws_df(fit$draws(c("alpha", "beta", "sigma")))
```
...
#| label: fig-marginals
#| fig-cap: >
#|   Marginal posterior distributions for the three parameters. Vertical
#|   dashed lines mark the true data-generating values (α = 1.20,
#|   β = 0.08, σ = 0.25). The posteriors are well-centred on the truth.
#| fig-width: 8
#| fig-height: 3.2
#| cache: true
#| code-fold: true

true_vals <- c(alpha = true_alpha, beta = true_beta, sigma = true_sigma)
...
draws |>
  ...
  geom_histogram(aes(y = after_stat(density)),
                 bins = 45, fill = "#2e7d6a", alpha = 0.7) +
  geom_density(colour = "#2e7d6a", linewidth = 0.9) +
  geom_vline(
    data = data.frame(
  ...
  labs(x = "Parameter value", y = "Density",
       ...
       subtitle = "Red dashed lines: true data-generating values")
```
...
cat("Posterior mean of beta:", round(mean(beta_draws), 3), "\n")
cat("Posterior SD of beta:", round(sd(beta_draws),   3), "\n")
cat("90% credible interval:",
    round(quantile(beta_draws, 0.05), 3), "to",
    round(quantile(beta_draws, 0.95), 3), "\n")
cat("P(beta > 0 | data):", round(mean(beta_draws > 0), 3), "\n")
```
...
### The regression line and its uncertainty
...
#| fig-cap: >
#|   Posterior regression lines. Each light line is one draw from the
#|   posterior for (α, β). The dark line is the posterior mean. The shaded
#|   band is the 90% credible interval for the conditional mean E[y|x].
#|   Data points are shown in teal.
#| fig-width: 6.5
#| fig-height: 4.2
#| cache: true
#| code-fold: true
...
# Sample 200 draws for line display
idx <- sample(nrow(draws), 200)

lines_df <- do.call(rbind, lapply(idx, function(i) {
  data.frame(
    x = x_seq,
    y = draws$alpha[i] + draws$beta[i] * x_seq,
    draw  = i
  )
}))

# Posterior mean line and 90% CI band
band_df <- data.frame(x = x_seq)
mu_mat <- outer(draws$alpha, rep(1, length(x_seq))) + outer(draws$beta,  x_seq)
band_df$mean <- colMeans(mu_mat)
band_df$lo <- apply(mu_mat, 2, quantile, 0.05)
band_df$hi <- apply(mu_mat, 2, quantile, 0.95)

ggplot() +
  geom_line(data = lines_df,
            aes(x, y, group = draw),
            colour = "#2e7d6a", alpha = 0.05, linewidth = 0.35) +
  geom_ribbon(data = band_df,
              aes(x, ymin = lo, ymax = hi),
              fill = "#2e7d6a", alpha = 0.20) +
  geom_line(data = band_df,
            aes(x, mean),
            colour = "#1a4a40", linewidth = 1.1) +
  geom_point(data = decomp,
             aes(x, y),
             colour = "#2e7d6a", alpha = 0.7, size = 1.8) +
  labs(
    x = "Temperature anomaly (°C, centred)",
    y = expression(log~decomposition~rate~(yr^{-1})),
    title = "Posterior regression lines",
    subtitle = "200 posterior draws (light), posterior mean (dark), 90% CI (band)"
  )
```
...
#| label: fig-ppc-dens
#| fig-cap: >
#|   Posterior predictive density overlay. The dark line is the observed
#|   data distribution; the light lines are 50 simulated datasets from the
#|   posterior. Good overlap indicates the normal linear model is a
...
ppc_dens_overlay(y, y_rep[1:50, ]) +
  labs(
    ...
    subtitle = "Dark: observed data. Light: 50 posterior predictive datasets"
  )
```
...
Two minor features are worth noting. First, the observed density peak is slightly sharper than the average replicated peak. This suggest that the model may spread predictions marginally wider than the data strictly require but it is a mild over-dispersion that is unlikely to affect inference on $\beta$.
Second, a handful of light lines show secondary bumps near 0.5 and 2.0 that the observed data do not exhibit; these are expected random fluctuations given only $N = 45$ observations, not evidence of misspecification. Overall, no systematic discrepancy is detectable and the normal likelihood is a defensible choice for these data.

```{r}
#| label: fig-ppc-intervals
#| fig-cap: >
#|   Posterior predictive intervals for each observation. Points are
#|   observed y values; intervals are 50% (thick) and 90% (thin) predictive
#|   intervals. Most observations fall within their predictive interval,
#|   consistent with a well-specified model.
#| fig-width: 7
#| fig-height: 4
#| code-fold: true
#| cache: true

ppc_intervals(y, y_rep, x = x, prob = 0.5, prob_outer = 0.9) +
  labs(
    x = "Temperature anomaly (°C, centred)",
    y = expression(log~k),
    title = "Posterior predictive intervals",
    subtitle = "50% (thick) and 90% (thin) predictive intervals"
  )
```
...
#| label: lm-compare
# Classical OLS fit
lm_fit <- lm(y ~ x, data = decomp)

# Stan posterior means
stan_means <- fit$summary(c("alpha", "beta", "sigma")) |>
  select(variable, mean, sd, q5, q95)

# OLS estimates and confidence intervals
lm_ci <- confint(lm_fit, level = 0.90)
lm_summary <- data.frame(
  variable = c("alpha", "beta", "sigma"),
  ols_est = c(coef(lm_fit)[["(Intercept)"]], coef(lm_fit)[["x"]], sigma(lm_fit)),
  ols_lo = c(lm_ci["(Intercept)", 1], lm_ci["x", 1], NA),
  ols_hi = c(lm_ci["(Intercept)", 2], lm_ci["x", 2], NA)
)

# Print side by side
merge(stan_means, lm_summary, by = "variable") |>
  knitr::kable(
    digits  = 3,
    caption = "Stan posterior vs OLS: estimates are nearly identical with weakly informative priors and N = 45.",
...
1. The prior is informative (small $N$, strong prior knowledge).
2. The model is hierarchical: OLS has no principled way to handle partial pooling.
3. The likelihood is non-Gaussian: OLS is no longer optimal.

All three situations arise in later chapters.
:::


## The complete workflow at a glance {#sec-workflow}

This workflow repeats in every model chapter. Memorise it now:

```{mermaid}
%%| fig-cap: "The complete Stan modelling workflow. Steps with a feedback arrow back to step 1 indicate situations that require revisiting the model specification: convergence failure (step 5) or a failing posterior predictive check (step 7)."
%%| cache: true
%%| label: fig-linear-wklfw
flowchart TD
...
1. **Parameter recovery.** Change the true slope to `true_beta <- 0.00` (no temperature effect). Re-run the full workflow. Does the 90% credible interval for $\beta$ contain zero? What is $P(\beta > 0 \mid \text{data})$? What would a frequentist analysis report?

2. **Prior sensitivity.** Refit the model with `beta ~ normal(0, 0.1)` (a tight prior near zero). How does the posterior mean for $\beta$ change? At what value of $N$ (try 10, 45, 200) does the prior stop mattering? Simulate datasets at each $N$ and compare.

3. **Uncentred predictor.** Refit with the raw (uncentred) temperature
   values. Look at the pair plot: how does the correlation between $\alpha$
   and $\beta$ change? Check `ess_bulk` for both fits. What does this tell
   you about why centring matters computationally?

4. **Predictive intervals.** Using the posterior draws, compute the 90%
   *predictive* interval for a new observation at $x = 3$ (a plot 3°C
   above the site mean). Compare this with the 90% credible interval for
   the *conditional mean* at $x = 3$. Why are they different widths?

   *Hint*: the predictive interval must account for $\sigma$; the credible
   interval for the mean does not.

   ```{r}
   #| eval: false
   x_new <- 3
   mu_new  <- draws$alpha + draws$beta * x_new  # mean at x_new
   y_new   <- rnorm(nrow(draws), mean = mu_new, sd = draws$sigma)  # new observation
   quantile(mu_new, c(0.05, 0.95))  # CI for mean
   quantile(y_new,  c(0.05, 0.95))  # predictive interval
   ```

5. **Extend to a second predictor.** Soil moisture (%) is another driver
   of decomposition. Add a second centred predictor `z` (soil moisture) to
   the Stan file: a new `vector[N] z` in the `data` block, a new `real
   gamma` parameter, and update the likelihood to
   `y ~ normal(alpha + beta * x + gamma * z, sigma)`. Simulate `z`
   correlated with temperature ($r \approx 0.4$) and refit.
   Does including soil moisture reduce $\sigma$?


## Further reading

- @gelman2020regression [Ch. 7-10]: thorough treatment of Bayesian
  regression, with worked examples close to the style of this chapter.
- @mcelreath2020rethinking [Ch. 4]: builds linear regression up from
  Gaussian priors and grid approximation before moving to Stan.
- @gelman2013bda [Ch. 14]: the technical treatment of Bayesian regression
  including conjugate analysis and the matrix form.
- Stan User's Guide, "Linear Regression" chapter:
